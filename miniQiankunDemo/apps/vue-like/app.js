 ...
 * ======================================================
 *
 * 这是一个不依赖 Vue 框架的"模拟 Vue"子应用，用于演示微应用生命周期。
 * 实际项目中这里会是真正的 Vue 3 应用。
 *
 * 关键点：
...
(function registerVueLikeApp(global) {
  const appName = "vue-like-app";
  let root = null;
  let propsCache = null;
  let state = {
    user: "Guest",
  ...
  function render(props = {}) {
    ...
    root = container.querySelector("#vue-like-root");

    // 如果容器不存在则创建
    if (!root) {
      root = document.createElement("div");
      root.id = "vue-like-root";
      container.appendChild(root);
    }

    // 渲染 HTML 模板
    root.innerHTML = `
      <section class="vue-like-card">
        <h3>${props.title || "Vue-like 子应用"}</h3>
        <p>这是一个不依赖 Vue 的模拟 Vue 子应用，用于演示微应用生命周期。</p>
        <p>全局用户：${state.user}</p>
        <p>全局主题：${state.theme}</p>
        <button id="vue-like-update-state">子应用修改全局状态</button>
      </section>
    `;

    // 绑定子应用内按钮事件 —— 子应用修改全局状态
    root.querySelector("#vue-like-update-state").onclick = () => {
  ...
  global[appName] = {
     ...
     */
    async bootstrap() {
      console.log("[vue-like-app] bootstrap");
    },
     ...
     */
    async mount(props) {
      console.log("[vue-like-app] mount", props);
      propsCache = props;
     ...
     */
    async unmount() {
      console.log("[vue-like-app] unmount");
      if (root) {
        root.innerHTML = "";
      }
      root = null;
     ...
     */
    async update(props) {
      console.log("[vue-like-app] update", props);
      propsCache = {
        ...propsCache,
        ...props,
      };
